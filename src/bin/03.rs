...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    Some(batteries.iter().map(|b| joltage(b)).sum())
}
...
pub fn part_two(input: &str) -> Option<u64> {
    let batteries = parse(input);
    Some(batteries.iter().map(|b| joltage12(b)).sum())
}
...
fn joltage_part(battery: &[u32]) -> (usize, u32) {
    debug_assert!(!battery.is_empty());
    let b1rev = battery.iter().rev().position_max().unwrap();
